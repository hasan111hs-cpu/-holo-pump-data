...
import csv, hashlib, io, json, re, socket, time, urllib.parse, urllib.request, urllib.error, zipfile
...
UA = {"User-Agent": "HOLO-PUMP-Quant-Lab/multicoin-universe-1.2.4"}
...
def _encode_url_path(url: str) -> str:
    """Percent-encode only the URL path; preserve an already-encoded query string."""
...
def get(url: str, retries: int = 6) -> bytes | None:
    # Archive object paths may contain non-ASCII symbol names. Encode the path
    # before urllib constructs the HTTP request. S3 listing query strings are
    # already encoded by urlencode() and are deliberately left untouched.
    encoded_url = _encode_url_path(url)
    last = None
    for i in range(retries):
        try:
            req = urllib.request.Request(encoded_url, headers=UA)
            ...
            if e.code != 429 and 400 <= e.code < 500:
                raise RuntimeError(f"Non-retryable HTTP {e.code}: {url}") from e
            last = e  # 429 and 5xx are transient/retryable
        except (urllib.error.URLError, TimeoutError, socket.timeout, ConnectionError) as e:
            last = e
        except (UnicodeEncodeError, ValueError) as e:
            # Deterministic request-construction failures must never be retried.
            raise RuntimeError(f"Non-retryable URL/request error: {url}: {e}") from e
        if i + 1 < retries:
            time.sleep(min(2 ** i, 30))
    raise RuntimeError(f"GET failed after {retries} transient attempts: {url}: {last}")
...
def _cache_path(key: str) -> Path:
    # Preserve the S3 key hierarchy, including Unicode symbol names.
...
def verified_zip(key: str, manifest: list[dict]) -> bytes:
    ...
    # CHECKSUM remains authoritative on every use, including cache hits.
    chk = get(url + ".CHECKSUM")
    if chk is None:
        raise RuntimeError(f"Missing CHECKSUM: {key}")
    try:
        # Binance .CHECKSUM files contain "<sha256>  <filename>", and the filename
        # carries the symbol name, which may be non-ASCII. Split on bytes and decode
        # only the hex digest, so a non-ASCII filename cannot break verification.
        expected = chk.split()[0].decode("ascii").lower()
    except (UnicodeDecodeError, IndexError) as e:
        raise RuntimeError(f"Malformed CHECKSUM: {key}") from e
    ...
    if cp.exists():
        ...
        if actual == expected:
            ...
            # Never trust stale/corrupt cache content. Remove it and obtain the
            # authoritative archive again; the replacement is still verified.
            cp.unlink()
            blob = get(url)
            if blob is None:
    ...
    manifest.append({
        "key": key, "bytes": len(blob), "sha256": actual,
        "checksum_verified": True, "source": source,
    })
    return blob
...
def main():
    ...
    # Working cache is never part of the research record. Guard locally against
    # accidental git inclusion without changing the repository's tracked .gitignore.
    git_exclude = Path(".git/info/exclude")
    if git_exclude.parent.exists():
        existing = git_exclude.read_text() if git_exclude.exists() else ""
        rule = "/_stage1_cache/"
    ...
    non_ascii_matches = audit_matches(lambda s: not s.isascii())
    ...
    selected_symbol_set = set(selected_months)
    for rec in non_ascii_matches:
        rec["ever_selected_top15"] = rec["symbol"] in selected_symbol_set

    symbols_with_2026_08_archive = sum(1 for sym in symbols if (spans[sym]["last_archive_month"] or "") >= "2026-08")
    print(f"Archive coverage audit: {symbols_with_2026_08_archive}/{len(symbols)} enumerated symbols have a 2026-08 archive")
    print(f"Non-ASCII archive symbols: {len(non_ascii_matches)}; selected at least once: {sum(1 for x in non_ascii_matches if x['ever_selected_top15'])}")
    ...
    universe = {
        ...
        "classification_constants": {
            ...
            "non_ascii_archive_symbols_not_excluded": non_ascii_matches,
        },
        "archive_symbols_enumerated": len(symbols),
        "archive_symbols_with_2026_08_archive": symbols_with_2026_08_archive,
